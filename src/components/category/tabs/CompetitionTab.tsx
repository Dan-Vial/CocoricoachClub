  ...
  return (
    <Tabs defaultValue="matches" className="space-y-4">
      <TabsList className="flex w-full overflow-x-auto no-scrollbar gap-1 h-auto flex-wrap md:flex-nowrap bg-muted">
        <TabsTrigger value="matches" className="flex items-center gap-1.5 text-xs sm:text-sm px-2 sm:px-3">
          <Swords className="h-4 w-4 shrink-0" />
          Matchs
        </TabsTrigger>
        {isRugby7 && (
          <TabsTrigger value="tournaments" className="flex items-center gap-1.5 text-xs sm:text-sm px-2 sm:px-3">
            <Trophy className="h-4 w-4 shrink-0" />
            Tournois
          </TabsTrigger>
        )}
        {isNationalTeam && (
          <TabsTrigger value="national-team" className="flex items-center gap-1.5 text-xs sm:text-sm px-2 sm:px-3">
